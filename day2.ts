...
type Play = keyof typeof ruleset;
...
const getRoundOutcome = (play: Play) => {
  const yourChoice = play.split(',')[1] as Move;
  return ruleset[play] + scores[yourChoice];
};

const getPlayBasedOnOutcome = (play: Play): Play => {
  const keys = Object.keys(ruleset) as Play[];
...
const getTotalPoints = (strategy: string) => {
  const plays: Play[] = strategy
    .split('\n')
    .map((x) => x.replace(' ', ',')) as Play[];

  let points = 0;
  plays.map((p) => {
    points += getRoundOutcome(p);
  });
  return points;
};

const getTotalPointsPart2 = (strategy: string) => {
  const plays: Play[] = strategy
    .split('\n')
    .map((x) => x.replace(' ', ',')) as Play[];
  ...
  let points = 0;
  plays.map((p) => {
    points += getRoundOutcome(getPlayBasedOnOutcome(p));
  });
  return points;
};

console.log(getTotalPoints(inputDay2));
console.log(getTotalPointsPart2(inputDay2));
